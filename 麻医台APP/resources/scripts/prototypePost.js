// 8.0.0.3358. Generated 9/28/2017 12:33:18 AM UTC
        ...
        query.each(function(dObj, elementId) {
            ...
            if(!itemId || itemReset) {
                ...
                if($ax.public.fn.IsVector(dObj.type) || $ax.public.fn.IsImageBox(dObj.type) || isDynamicPanel || $ax.public.fn.IsLayer(dObj.type)) {
                    ...
                    else if(itemReset) $axElement.enabled(true); // resetting needs to make sure to set back values to default


                    // Initialize selected elements
                    if(dObj.selected) $axElement.selected(true);
                    else if(itemReset) $axElement.selected(false);
                }
...
$axure.internal(function($ax) {
    ...
    var _refreshRepeater = function(repeaterId, eventInfo, refreshType, itemsPregen) {
        ...
        if(itemsPregen) {
            ...
            for(var i = 0; i < ids.length; i++) $ax.initializeObjectEvents($ax('#' + ids[i]), refreshType);
        }
