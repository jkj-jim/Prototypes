// 8.0.0.3357. Generated 9/8/2017 11:47:41 PM UTC
...
$axure.internal(function($ax) {
    ...
    _repeaterManager.refreshAllRepeaters = function() {
        $ax('*').each(function(diagramObject, elementId) {
            ...
            $ax.style.updateStateClass(elementId); // Only do this when refreshing all, state is reset when doing regular refreshes
        });
    };
...
$axure.internal(function($ax) {
    ...
    _style.updateStateClass = function (repeaterId) {
        ...
        for (var i = 0; i < subElementIds.length; i++) {
            var elementId = subElementIds[i];
            var state = $ax.style.generateState(elementId);
            $jobj(elementId).addClass(state);
            $jobj(elementId + '_div').addClass(state);

            _applyImageAndTextJson(elementId, state);
        }
    }
